    ...
    function addDatePicker() {
        var n, x = document.getElementsByClassName("inputdate");
        ...
        if ((thesetodos === null) || (thesetodos === "") || (thesetodos === undefined)) {
            $("#todo-lists-with-datepicker").lobiList({
                ...
                afterListAdd: function(lobilist, list){
                    var $startDateInput = list.$el.find('form [name=startDate]');
                    $startDateInput.datepicker();
                    var $dueDateInput = list.$el.find('form [name=dueDate]');
                    $dueDateInput.datepicker();
                    var $completeDateInput = list.$el.find('form [name=completeDate]');
                    $completeDateInput.datepicker();
                }
            });
        } else {
...
function gatherToDos() {
    "use strict";
    var i, j, wrapcolor, todoheader, todotitle, listitem, itemtitle, itemdescription, itemstartdate, itemduedate, itemcompletedate, itemdone, wraptodos = "", wraplists = [], wrapitems = [], exptodos = "";
    ...
    for (j = 0; j < wrapper.length; j+=1) {
        ...
        for (i = 0; i < listitem.length; i+=1) {
            ...
            itemstartdate = listitem[i].querySelectorAll(".lobilist-item-startdate");
            if (itemstartdate.length > 0) {
                exptodos = exptodos + ', "startDate": "' + itemstartdate[0].innerHTML + '"';
            }
            ...
            itemcompletedate = listitem[i].querySelectorAll(".lobilist-item-completedate");
            if (itemcompletedate.length > 0) {
                exptodos = exptodos + ', "completeDate": "' + itemcompletedate[0].innerHTML + '"';
            }
